...
import { ArrowLeft, TrendingUp, TrendingDown, Trophy, Lock } from "lucide-react";
...
export default function PlayerProfile() {
  ...
  const { player: loggedPlayer } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      // Fetch player, matches, and tournament registrations
      const [pRes, m1Res, m2Res, regsRes] = await Promise.all([
      ...
      // Collect all tournament IDs (from matches + registrations)
  ...
  // Include tournaments the player registered for but hasn't played in yet
      ...
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        ...
        <div className="glass-card p-6 mb-6">
          <div className="flex items-start justify-between">
            ...
            {isOwnProfile && (
              <button
                ...
                className="text-xs flex items-center gap-1 px-2 py-1 rounded border border-border text-muted-foreground hover:text-foreground transition-colors"
              >
          ...
          {showPasswordForm && isOwnProfile && (
            <form onSubmit={handlePasswordChange} className="mt-4 p-4 rounded-md bg-muted/30 space-y-2.5">
              <div>
        ...
        <h2 className="font-heading font-semibold text-foreground mb-3 flex items-center gap-2">
          <Trophy className="w-4 h-4" /> Torneos Participados ({Object.keys(matchesByTournament).length})
        </h2>
